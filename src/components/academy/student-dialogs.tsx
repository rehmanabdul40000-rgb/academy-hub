      ...
      >
        <div className="flex items-center justify-between border-b border-border pb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 id="collect-payment-title" className="font-display font-semibold text-foreground">
                Collect Payment
              </h3>
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          {lastPayment ? (
            <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3 text-xs text-emerald-300">
              Payment saved successfully. The receipt is ready to print.
            </div>
          ) : null}
          {/* Fee Overview Pill */}
          ...
          <div className="flex flex-wrap justify-end gap-2.5 pt-2">
            <Button type="button" variant="outline" size="sm" onClick={onClose}>
              {lastPayment ? "Close" : "Cancel"}
            </Button>
            {lastPayment ? (
              <Button
                type="button"
                size="sm"
                onClick={() => printPaymentReceipt(student, lastPayment)}
              ...
              <Button
                ...
                className="bg-cyan-600 text-white hover:bg-cyan-500"
              >
                {isSavingPayment ? "Saving..." : "Confirm Payment"}
              </Button>
            )}
          </div>
        </form>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <input
                ...
                className="academy-date-input mt-1 h-10 w-full cursor-pointer rounded-lg border border-input bg-background px-3 text-sm outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20"
              />
            </div>
          </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="mt-5 space-y-4 text-sm">
          ...
          {getPaymentHistory(student).length > 0 && (
            <section className="mt-4 rounded-lg border border-border bg-muted/20 p-3.5">
              <div className="flex items-center justify-between">
                <h4 className="text-xs font-semibold uppercase tracking-wider text-cyan-400">
                  Payment History
                </h4>
                <span className="text-[11px] text-muted-foreground">
                  {getPaymentHistory(student).length} payment(s)
                </span>
              </div>
              <div className="mt-3 space-y-2">
                {getPaymentHistory(student).map((payment) => (
                  <div
                    key={payment.id}
                    className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border/70 bg-card p-2.5 text-xs"
                  >
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        {payment.paymentDate}, {payment.paymentTime} · {payment.id}
                      </p>
                    </div>
                    <Button
                      type="button"
                      variant="ghost"
                      size="sm"
                      onClick={() => printPaymentReceipt(student, payment)}
                      className="h-7 gap-1 text-[11px] text-cyan-400 hover:text-cyan-300"
                    >
...
export function DeleteStudentDialog({
  ...
  useEffect(() => {
    ...
    setRecoveryId(beginStudentDeletionSnapshot(student));
  }, [open, student]);
  ...
  function handleCancel() {
    if (recoveryId) cancelStudentDeletionSnapshot(recoveryId);
    onClose();
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        ...
        className="relative w-full max-w-sm rounded-xl border border-destructive/30 bg-card p-6 shadow-2xl"
      >
        <div className="flex items-center gap-3">
