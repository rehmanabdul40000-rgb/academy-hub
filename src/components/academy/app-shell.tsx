...
import { BookOpen, FileSpreadsheet, GraduationCap, LayoutDashboard, LogOut, Menu, Plus, Search, Settings, UserPlus, Users, X, UserRound, UserRoundCheck, Sun, Moon, Clock3, KeyRound, ShieldCheck, Volume2, VolumeX } from "lucide-react";
...
import { isAgentVoiceEnabled, setAgentVoiceEnabled, stopAgentVoice } from "@/features/agent/agent-voice";
import { Route as RootRoute } from "@/routes/__root";
import { AcademyAgent } from "@/components/academy/academy-agent";
...
export function AppShell({ children, title, subtitle, onSearchChange, searchValue, showExport = true, showAddStudent = true }: { children: ReactNode; title: string; subtitle: string; onSearchChange?: (val: string) => void; searchValue?: string; showExport?: boolean; showAddStudent?: boolean; }) {
  ...
  const [theme, setTheme] = useState(getWorkspaceTheme()); const [agentVoiceEnabled, setAgentVoiceEnabledState] = useState(isAgentVoiceEnabled());
  ...
  useEffect(() => {
    ...
    const onAgentVoice = (e: Event) => setAgentVoiceEnabledState((e as CustomEvent<boolean>).detail ?? isAgentVoiceEnabled());
    ...
    window.addEventListener("academy-agent-voice-updated", onAgentVoice);
    return () => {
      ...
      window.removeEventListener("academy-agent-voice-updated", onAgentVoice);
    };
  }, []);
  ...
  return <><div className={`${theme} min-h-dvh w-full min-w-0 overflow-x-hidden bg-background text-foreground`}><aside className="fixed inset-y-0 left-0 z-30 hidden w-64 border-r border-sidebar-border print:hidden lg:block">{sidebar}</aside>{open ? <div className="fixed inset-0 z-40 lg:hidden"><button aria-label="Close navigation" className="absolute inset-0 bg-background/80 backdrop-blur-sm" onClick={() => setOpen(false)} /><aside className="relative h-full w-72 border-r border-sidebar-border bg-sidebar shadow-2xl print:hidden">{sidebar}<Button variant="ghost" size="icon" onClick={() => setOpen(false)} className="absolute right-3 top-3 text-muted-foreground hover:text-foreground"><X className="size-5" /></Button></aside></div> : null}<div className="min-w-0 lg:pl-64"><header className="sticky top-0 z-20 flex min-w-0 flex-wrap items-center justify-between gap-2 overflow-x-hidden border-b border-border bg-background/85 px-2 py-2 backdrop-blur-xl print:hidden sm:h-16 sm:flex-nowrap sm:gap-0 sm:px-6 sm:py-0 lg:px-8"><div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3"><Button variant="ghost" size="icon" onClick={() => setOpen(true)} className="lg:hidden"><Menu className="size-5" /></Button><div className="flex min-w-0 items-center gap-1.5 sm:gap-2"><BookOpen className="size-4 text-cyan-400" /><span className="hidden font-display text-sm font-semibold tracking-wide text-foreground sm:inline">{academyName}</span><span className="hidden text-muted-foreground sm:inline">•</span><span className="text-xs font-medium text-muted-foreground">Workspace</span><span className="mx-1 h-4 w-px bg-border" /><button type="button" title={agentVoiceEnabled ? "Turn off agent voice" : "Turn on agent voice"} aria-label={agentVoiceEnabled ? "Turn off agent voice" : "Turn on agent voice"} onClick={() => { const next = !agentVoiceEnabled; setAgentVoiceEnabled(next); setAgentVoiceEnabledState(next); if (!next) stopAgentVoice(); }} className={`grid size-7 place-items-center rounded-md border border-border transition-colors ${agentVoiceEnabled ? "bg-cyan-950/40 text-cyan-400" : "text-muted-foreground hover:bg-accent/60 hover:text-foreground"}`}>{agentVoiceEnabled ? <Volume2 className="size-3.5" /> : <VolumeX className="size-3.5" />}</button><div className="flex items-center gap-1 rounded-lg border border-border bg-card/60 p-0.5" aria-label="Theme"><button type="button" title="Light theme" aria-label="Light theme" onClick={() => applyWorkspaceTheme("light")} className={`grid size-7 place-items-center rounded-md transition-colors ${theme === "light" ? "bg-accent text-cyan-600 shadow-sm" : "text-muted-foreground hover:bg-accent/60 hover:text-foreground"}`}><Sun className="size-3.5" /></button><button type="button" title="Dark theme" aria-label="Dark theme" onClick={() => applyWorkspaceTheme("dark")} className={`grid size-7 place-items-center rounded-md transition-colors ${theme === "dark" ? "bg-accent text-cyan-400 shadow-sm" : "text-muted-foreground hover:bg-accent/60 hover:text-foreground"}`}><Moon className="size-3.5" /></button></div></div></div><div className="flex shrink-0 items-center gap-1.5 sm:gap-3"><form onSubmit={handleHeaderSearchSubmit} className="relative hidden md:block"><Search className="pointer-events-none absolute left-3 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" /><input type="text" placeholder="Search students..." value={searchValue !== undefined ? searchValue : headerSearch} onChange={(e) => onSearchChange ? onSearchChange(e.target.value) : setHeaderSearch(e.target.value)} className="h-9 w-44 rounded-lg border border-input bg-card/60 pl-8 pr-3 text-xs outline-none transition-all placeholder:text-muted-foreground focus:w-60 focus:border-cyan-500" /></form>{showExport && can("reports") && <Button variant="outline" size="sm" onClick={handleExport} disabled={isExporting} className="h-9 gap-1.5 border-cyan-500/30 bg-cyan-950/30 text-cyan-400 hover:bg-cyan-900/40"><FileSpreadsheet className="size-4" /><span className="hidden sm:inline">{isExporting ? "Exporting..." : "Export to Excel"}</span><span className="sm:hidden">Excel</span></Button>}{showAddStudent && (isAdmin || session?.permissions?.studentsSave) && <Button asChild size="sm" className="h-9 gap-1.5 bg-cyan-600 text-white hover:bg-cyan-500"><Link to="/add-student"><Plus className="size-4" /><span className="hidden sm:inline">Add Student</span></Link></Button>}<Link to="/settings" className="grid size-9 place-items-center rounded-full border border-cyan-500/40 bg-cyan-950/40"><span className="text-xs font-bold text-cyan-300">{getInitials(adminName)}</span></Link></div></header><main className="mx-auto w-full min-w-0 max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-8"><div className="mb-6"><h1 className="font-display text-2xl font-bold tracking-tight sm:text-3xl text-foreground">{title}</h1><p className="mt-1 text-sm text-muted-foreground">{subtitle}</p></div>{children}</main></div></div><AcademyAgent /></>;
}
