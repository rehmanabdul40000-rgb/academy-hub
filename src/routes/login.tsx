...
function LoginPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const expectedUsername = currentSettings.adminUsername.trim().toLowerCase();
      const expectedPassword = currentSettings.adminPassword ?? "";
    ...
    >
      ...
      <section className="login-card relative z-10 w-full max-w-[29rem] overflow-hidden rounded-[1.75rem] border border-login-border bg-login-card p-6 shadow-login backdrop-blur-2xl sm:p-9">
        ...
        <header className="text-center">
          ...
          <h1 className="font-display text-2xl font-semibold tracking-tight text-login-heading sm:text-[1.75rem]">
            Welcome Back
          </h1>
        ...
        <form className="mt-7 space-y-4" onSubmit={handleSubmit} noValidate>
          <div className="space-y-1.5">
            ...
            <div className="login-input-wrap flex h-11 items-center gap-3 rounded-xl border border-login-border bg-login-input px-3.5 transition duration-200 focus-within:border-cyan-500 focus-within:ring-2 focus-within:ring-cyan-500/20">
              ...
              <input
                ...
                className="login-input min-w-0 flex-1 bg-transparent text-sm text-login-heading outline-none placeholder:text-login-placeholder disabled:cursor-wait"
              />
            </div>
          </div>
          <div className="space-y-1.5">
          ...
          <Button
            type="submit"
            disabled={isSubmitting}
            className="login-button h-11 w-full rounded-xl border border-login-button-border bg-login-button text-sm font-semibold text-login-button-foreground shadow-login-button transition duration-200 hover:bg-login-button-hover hover:shadow-login-button-hover active:translate-y-px disabled:cursor-wait disabled:opacity-70"
          >
