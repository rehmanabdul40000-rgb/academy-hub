...
import type { FeeMetrics, NewStudentInput, PaymentStatus, Student } from "@/types/student";
...
function saveStudentsList(students: Student[]): void {
  if (typeof window === "undefined") return;
  try {
...
export function addStudent(input: NewStudentInput): {
  ...
  const newRecord: Student = {
    ...
    savedAt: input.savedAt || savedAtFormatted,
    dateAdded: input.dateAdded || dateAddedFormatted,
    timeAdded: input.timeAdded || timeAddedFormatted,
    createdAt: nowIso,
    updatedAt: nowIso,
  };
...
export function recordQuickPayment(
  ...
  return updateStudent(id, { amountPaid: newPaid });
}
...
export function calculateMetrics(students: Student[]): FeeMetrics {
  ...
  for (const s of students) {
    ...
    const remaining = Number(s.remainingFees) || 0;
